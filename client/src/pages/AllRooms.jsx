...
const CheckBox = ({ label, selected = false, onchange = () => { } }) => {
  return (
    <div className='flex items-center gap-2 cursor-pointer mt-2 text-sm'>
      <input type="checkbox" checked={selected} onChange={(e) => onchange(e.target.checked, label)} className='w-4 h-4' />
...
const RadioButton = ({ label, selected = false, onchange = () => { } }) => {
  return (
    <div className='flex items-center gap-2 cursor-pointer mt-2 text-sm'>
      <input type="radio" checked={selected} onChange={(e) => onchange(e.target.checked, label)} className='w-4 h-4' />
      <span className='font-light select-none'>{label}</span>
    </div>
  )
}
const AllRooms = () => {
  ...
  const priceRanges = ['0 to 500', '500 to 1000', '1000 to 3000', ' 3000 to 5000'];
  ...
  //function to check room matches selected price ranges
  ...
  // Filter Rooms based on selected filters and search params
  const filteredRoomsData = useMemo(() => {
    return rooms.filter((room) => 
       matchesRoomType(room) && matchesPriceRange(room) && filteredRooms(room)
    ).sort(sortRooms);
  ...
  return (
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 lg:px-24 xl:px-32'>

      <div>
        <div className='flex flex-col items-start text-left'>
          <h1 className='font-playfair text-4xl md:text-[40px]'>Hotel Rooms</h1>
          <p className="text-sm md:text-base text-gray-500/90 mt-2 max-w-174">Take Advantage of our limited-time offers and book your dream room today!</p>
        </div>
        {filteredRoomsData.map((room, index) => (
      ...
      <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
        <div className={`flex items-center justify-between px-5 py-3 ${openFilters && "border-b"} border-gray-300`}>
        ...
        <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-300`}>
          <div className='px-5 pt-5'>
            ...
            {roomTypes.map((type, index) => (
              <CheckBox key={index} label={type} selected={selectedFilters.roomType.includes(type)} onChange={(checked) => handleFilterChange(checked, type,'roomType')} />
            ))}
          </div>
          <div className='px-5 pt-5'>
          ...
          <div className='px-5 pt-5'>
              <p className='font-medium text-gray-800 pb-2'>Sort By</p>
              {
                sortOptions.map((option, index) => (
                  <RadioButton key={index} label={option} onChange={() => handleSortChange(option)} />
                ))
              }
            </div>
...
const AllRooms = () => {
  ...
  return (
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 lg:px-24 xl:px-32'>
      ...
      {/* Left Content - Room Cards */}
      <div>
        <div className='flex flex-col items-start text-left'>
          <h1 className='font-playfair text-4xl md:text-[40px]'>Hotel Rooms</h1>
          <p className="text-sm md:text-base text-gray-500/90 mt-2 max-w-174">
            Take advantage of our limited-time offers and book your dream room today!
          </p>
        </div>
          ...
          <div className='px-5 pt-5'>
            ...
            {roomTypes.map((type, index) => (
              <CheckBox
                ...
                onChange={(checked) => handleFilterChange(checked, 'roomType', type)}
              />
            ))}
          </div>
          ...
          <div className='px-5 pt-5'>
            ...
            {priceRanges.map((range, index) => (
              <CheckBox
                ...
                onChange={(checked) => handleFilterChange(checked, 'priceRange', range)}
              />
            ))}
          </div>
