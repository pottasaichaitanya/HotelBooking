...
const HotelCard = ({room,index}) => {
  return (
    <Link to={`/rooms/${room._id}`} onClick={()=>scrollTo(0,0)} key={room._id}className={`flex flex-col gap-4 bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow duration-300 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}`}>
